...
Эффективность обогащения рассчитывается по формуле **Recovery = (Cx(F-T))/(Fx(C-T))x100%**, где:  
...
Итоговая метрика складывается из двух величин: 25% x sMAPE(rougher) + 75% x sMAPE(final)
